...
const menuFormatter = (menuItems) => {
  if (menuItems.length > 0) {
    const formatted = menuItems.map((item) => ({
      title: item.menuName,
      path: item.url,
      icon: ICONS.label,
      children: item.submenu.length > 0 ? menuFormatter(item.submenu) : null,
    }));
...
(async () => {
  const response = await axios.get(`/v1/menu/list`, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
      'Content-Type': 'application/json',
    },
  });
  const structuredMenuList = menuFormatter(response.data);

  await menuItem.push(...structuredMenuList);
})();
      ...
      items: [
        {
          ...
          icon: ICONS.master,
          children: [
            { title: 'Institute', path: PATH_DASHBOARD.master.instituteList },
          ],
        },
      ],
    },
